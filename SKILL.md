...
| 主力资金流 | 东财 → 雪球 → 同花顺回退 | 暂停 | 暂停 | 暂停 |
| 龙虎榜 | 暂停：旧外部源已禁用 | ✗ | ✗ | ✗ |
| 新闻情感分析 | akshare 新闻 → LLM 优先 / 本地规则降级 | akshare 新闻 → LLM 优先 / 本地规则降级 | ✗ ETF 无个股新闻 | Finnhub 新闻 → LLM 优先 / 本地规则降级 |
...
- `--out`：输出目录，缺省 `./output`
- `--no-llm`：关闭可选 LLM；新闻情感自动使用本地可解释规则模型，核心分析与 PDF 照常生成
...
>
> **可选 LLM 增强**：行情、技术指标、因子与 PDF 渲染不依赖 LLM。设置 `STOCK_ANALYST_LLM_PROVIDER=disabled` 可明确关闭；默认 `auto` 会依次识别 Anthropic 或 OpenAI 兼容配置。未配置凭证时，新闻情感自动回退到本地规则模型；同行推荐和综合解读跳过。
...
| 新闻情感 | LLM / 本地规则模型 | LLM 优先；未配置或调用失败时本地去重、关键词加权、重要性和时效衰减 |
| A 股资金流 | 东财 / 雪球 / 同花顺 | 东财历史日资金流优先，其余作为回退；缺失不等同于净流入为零 |

龙虎榜、期权流、公告信息面仍未接入；A 股资金流已恢复多源回退。
...
```
当日涨跌 + 技术指标(×0.7) + 新闻情感(LLM/本地规则, ×置信度)
    + 多因子合成(仅技术因子, weighted ×0.8) + A股资金流
    + 龙虎榜(暂停,0) + 期权流(暂停,0) + 信息面(暂停,0)
↓
...
- 本地新闻情感是规则模型，对反讽、复杂上下文和谣言识别有限；LLM 可用时优先使用 LLM
- A 股资金流依赖外部公开接口，可能受风控或口径差异影响；龙虎榜、期权流、公告/SEC 信息面仍未接入
- ETF 当前无新闻情感，信号偏少
